            ...
            steps { 
                script {
                    withCredentials([usernamePassword(credentialsId: 'nexus-user', usernameVariable: 'USERNAME', passwordVariable: 'PASSWORD')]){
                        sh 'mvn deploy -DrepositoryId=jenkins -DaltDeploymentRepository=nexus::default::http://192.168.56.3:8091/repository/maven-releases/ -Dnexus.user=${USERNAME} -Dnexus.password=${PASSWORD} -X'
                    }
                } 
            } 
        }
